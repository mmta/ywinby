...
#[command(
  ...
  A system that keeps one share of the 2-of-3 Shamir's secret sharing system,\n\
  and release it to the recipient if the owner fails respond after a certain time."
)]
struct Args {
  ...
  push_privkey: String,
  /// Public key of the above to be used by web clients, will be written to runtime-config.json.
  #[arg(
    short('p'),
    long,
    env,
  ...
  push_pubkey: String,
  /// The URL that web clients use to contact this server, will be written to runtime-config.json.
  ...
  /// Activate serverless mode, and authenticate request for scheduled task using this token
...
fn update_client_config(api_path: String, pubkey: String) -> Result<(), std::io::Error> {
  let cfg = WebClientRuntimeConfig {
    api_url: api_path,
    push_pubkey_base64: pubkey,
  };
...
async fn main() -> std::io::Result<()> {
  ...
  HttpServer::new(|| {
    App::new()
      ...
      .data_factory(|| async {
        let state = data_factory_creator().await;
        Ok::<_, ()>(state)
      })
